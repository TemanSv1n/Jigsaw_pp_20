...
public class TeapotCannonItem extends Item implements CustomArmPoseItem {
    public TeapotCannonItem() {
        super(new Properties().durability(135));
    }
...
//                    itemstack.hurtAndBreak(1, entity, e -> e.broadcastBreakEvent(entity.getUsedItemHand()));
                ItemStack stack = ProjectileWeaponItem.getHeldProjectile(entity, e -> e.getItem() == ModItems.TNTPOT.get());
                if (stack == ItemStack.EMPTY) {
                    for (int i = 0; i < entity.getInventory().items.size(); i++) {
                        ItemStack teststack = entity.getInventory().items.get(i);
                        if (teststack != null && teststack.getItem() == ModItems.TNTPOT.get()) {
                            stack = teststack;
                            break;
                        }
                    }
                ...
                if (entity.getAbilities().instabuild || stack != ItemStack.EMPTY) {
                    ...
                    TeapotMissileEntity entityarrow = TeapotMissileEntity.shoot(world, entity, new Random(), 1.5f, 1, 1);
                    itemstack.hurtAndBreak(1, entity, e -> e.broadcastBreakEvent(entity.getUsedItemHand()));

                    if (entity.getAbilities().instabuild) {
                        entityarrow.pickup = AbstractArrow.Pickup.CREATIVE_ONLY;
                    } else {
                        if (new ItemStack(ModItems.TNTPOT.get()).isDamageableItem()) {

...
public class PoopOverlay {
    ...
    private static final ResourceLocation POOP = new ResourceLocation(JigsawPpMod.MODID,
            "textures/gui/poop_overlay.png");
    ...
    public static final IGuiOverlay HUD_POOP = ((gui, guiGraphics, partialTick, screenWidth, screenHeight) -> {
        ...
        if(renderingCondition(mc.player)) {
            renderTextureOverlay(guiGraphics, POOP, 1.0F, screenWidth, screenHeight);
        }
    ...
    public static boolean renderingCondition(Entity entity) {
        if (entity == null)
            return false;
        if (entity instanceof Player _livEnt0 && _livEnt0.hasEffect(ModEffects.POOP.get())) {
            return true;
        }
        return false;
    }
